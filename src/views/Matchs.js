...
function Matchs() {
  ...
  const updateMatchs = ()=> {
    matchService.getMatchs().then((res) => {
  ...
  return (
    <Container>
      <Title>Matchs</Title>
      <IconButton color="primary" aria-label="Refresh" onClick={()=>{updateMatchs()}}>
        <RefreshIcon />
      </IconButton>
        ...
        matchs.map((match) => {
          return (
              <Grid container alignItems={"center"}  spacing={2}>
                ...
                <Grid item >
                  <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                    {match.pointage.jeu[2] && <Grid item>
                      <p>{match.pointage.jeu[2][0]}</p>
                    </Grid>}
                    {match.pointage.jeu[1] && <Grid item x>
                      <p>{match.pointage.jeu[1][0]}</p>
                    </Grid>}
                    {match.pointage.jeu[0] && <Grid item >
                  ...
                  <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} >
                    ...
                    {match.pointage.jeu[1] &&<Grid item >
